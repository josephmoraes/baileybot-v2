...
process.env.CAMPAIGN_DELAY_MS = "0";
...
test("importa e exporta clientes em Excel", () => {
    ...
    const resultado = excelService.importar(base64);
    assert.equal(resultado.importados, 2);
    assert.ok(userService.buscarPorCodigo("CLI-001"));
    const repetido = excelService.importar(base64);
