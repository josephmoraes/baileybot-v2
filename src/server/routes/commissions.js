...
import pdfService from "../../services/commissionPdfService.js";
const router=Router();
...
router.get("/technician-inquiries",(q,s)=>enviar(s,()=>service.listarConsultasTecnicos()));
router.post("/technician-inquiries",(q,s)=>enviar(s,()=>service.registrarConsultaTecnico(q.body),201));
router.delete("/technician-inquiries/:id",(q,s)=>enviar(s,()=>service.excluirConsultaTecnico(q.params.id)));
...
router.get("/requests/:id/pdf",async(q,s)=>{try{let arquivo=service.obterPdfSolicitacao(q.params.id);if(!arquivo){const solicitacao=service.obterSolicitacao(q.params.id);const pdf=await pdfService.gerar(solicitacao);service.salvarPdfSolicitacao(q.params.id,pdf);arquivo=service.obterPdfSolicitacao(q.params.id);}s.setHeader("Content-Type",arquivo.mimeType);s.setHeader("Content-Length",arquivo.data.length);s.setHeader("Content-Disposition",`attachment; filename="${arquivo.filename}"`);s.send(arquivo.data);}catch(e){console.error(e);s.status(400).json({error:e.message});}});
router.post("/requests",async(q,s)=>{let solicitacao;try{solicitacao=service.criarSolicitacao(q.body);if(!q.body.draft){const completa=service.obterSolicitacao(solicitacao.id);const pdf=await pdfService.gerar(completa);const arquivo=service.salvarPdfSolicitacao(solicitacao.id,pdf);solicitacao.pdf={available:true,filename:arquivo.filename,url:`/api/commissions/requests/${solicitacao.id}/pdf`};}s.status(201).json(solicitacao);}catch(e){if(solicitacao&&!q.body.draft)service.excluirSolicitacaoIncompleta(solicitacao.id);console.error(e);s.status(400).json({error:e.message});}});
