...
function renderizarEtiquetasCliente(selecionadas = []) {
    ...
    if (!area) return;
    area.innerHTML = clientesTags.map(tag => `<label class="reactivation-tag-choice"><input type="checkbox" value="${tag.id}" ${selecionadas.includes(tag.id) ? "checked" : ""}><span style="--tag-color:${fichaSeguro(tag.color)}">${fichaSeguro(tag.name)}</span></label>`).join("");
}
...
async function carregarClientes(page = clientesPagina) {
    ...
    try {
        ...
        const parametros = new URLSearchParams({ page, perPage: 50, search: busca,
            priority: document.getElementById("filtroClientePrioridade")?.value || "todos",
            status: document.getElementById("filtroClienteStatus")?.value || "todos",
            seller: document.getElementById("filtroClienteVendedor")?.value || "todos",
        ...
        [["filtroClienteStatus", dados.filters?.statuses], ["filtroClienteVendedor", dados.filters?.sellers]].forEach(([id, options]) => {
            const select = document.getElementById(id); if (!select || select.dataset.loaded) return;
            select.insertAdjacentHTML("beforeend", (options || []).map(option => `<option value="${fichaSeguro(option)}">${fichaSeguro(option)}</option>`).join(""));
...
function fichaGrafico(serie) {
    if (!serie?.length) return '<div class="app-state py-4"><p class="mb-0">Sem movimentações reais para esta visão.</p></div>';
    const largura = Math.max(720, serie.length * 105); const altura = 260; const margem = 42;
    const valores = serie.map(item => Number(item.value) || 0); const max = Math.max(1, ...valores); const min = Math.min(...valores);
    const pontos = serie.map((item, index) => ({ ...item, x: margem + index * ((largura - margem * 2) / Math.max(1, serie.length - 1)), y: altura - margem - (Number(item.value) / max) * (altura - margem * 2) }));
    const maior = Math.max(...valores); const menor = Math.min(...valores);
    const variacoes = pontos.filter(item => item.variation !== null); const crescimento = [...variacoes].sort((a, b) => b.variation - a.variation)[0]; const queda = [...variacoes].sort((a, b) => a.variation - b.variation)[0];
...
async function carregarFichaCompletaCliente(id) {
    ...
    const conteudo = document.getElementById("cliente360Conteudo") || document.getElementById("fichaClienteConteudo");
    if (!conteudo) return;
    if (modalElement) bootstrap.Modal.getOrCreateInstance(modalElement).show();
    conteudo.innerHTML = '<div class="text-center py-5"><span class="spinner-border"></span></div>';
    try {
        ...
        const titulo = document.getElementById("cliente360Titulo") || document.getElementById("fichaClienteTitulo");
        titulo.textContent = client.company_name || client.name || `Cliente ${client.customer_code}`;
        ...
        conteudo.innerHTML = `
          ...
          <div class="row g-3 mb-4">${[["Última compra",fichaData(client.analytics.ultimaCompra)],["Dias sem comprar",client.analytics.diasSemComprar ?? "—"],["Ticket médio",fichaMoeda(client.analytics.ticketMedio)],["Faturamento",fichaMoeda(client.analytics.faturamento)],["Frequência média",client.analytics.frequenciaMediaDias === null ? "—" : `${client.analytics.frequenciaMediaDias} dias`],["Quantidade de compras",Number(client.analytics.compras).toLocaleString("pt-BR")]].map(([label,value]) => `<div class="col-md-4 col-xl-2"><div class="client-detail"><small>${label}</small><strong>${value}</strong></div></div>`).join("")}</div>
        ...
        conteudo.querySelectorAll("[data-chart-view]").forEach(button => button.addEventListener("click", () => {
            conteudo.querySelectorAll("[data-chart-view]").forEach(item => item.classList.remove("active")); button.classList.add("active");
            document.getElementById("cliente360Grafico").innerHTML = fichaGrafico(cliente360Atual.analytics.series[button.dataset.chartView]);
        }));
    } catch (error) {
...
function inicializarCliente360() {
    const id = Number(location.hash.match(/^#\/clientes\/360\/(\d+)$/)?.[1] || window.sessionStorage.getItem("cliente360Id"));
    if (!id) return document.getElementById("cliente360Conteudo").innerHTML = '<div class="alert alert-warning">Selecione um cliente na lista.</div>';
...
async function salvarCliente() {
    ...
    const tag_ids = [...document.querySelectorAll("#clienteEtiquetas input:checked")].map(input => Number(input.value));
...
async function inicializarClientes() {
    try {
        ...
        if (!respostaTags.ok) throw new Error(clientesTags.error || "Não foi possível carregar as etiquetas.");
        renderizarEtiquetasCliente([]);
    } catch (erro) {
        console.error(erro);
    }
    ...
    ["filtroClientePrioridade", "filtroClienteStatus", "filtroClienteVendedor", "filtroClienteAtivo", "ordenacaoClientes"]
        .forEach(id => document.getElementById(id)?.addEventListener("change", () => carregarClientes(1)));
    ["filtroClienteDiasMin", "filtroClienteDiasMax"].forEach(id => document.getElementById(id)?.addEventListener("input", () => {
        clearTimeout(clientesPesquisaTimer); clientesPesquisaTimer = setTimeout(() => carregarClientes(1), 250);
    }));
    document.getElementById("limparFiltrosClientes")?.addEventListener("click", () => {
        ["pesquisaCliente", "filtroClienteDiasMin", "filtroClienteDiasMax"].forEach(id => { document.getElementById(id).value = ""; });
        ["filtroClientePrioridade", "filtroClienteStatus", "filtroClienteVendedor", "filtroClienteAtivo"].forEach(id => { document.getElementById(id).value = "todos"; });
        document.getElementById("ordenacaoClientes").value = "az"; carregarClientes(1);
    });
